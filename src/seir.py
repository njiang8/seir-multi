...
class World(object):
    def __init__(self, population_size):
        # Number of Agnets
        self.numberOfAgent = population_size
        # Generate Agents list
        #self.agent = [Agent(self) for i in range(self.numberOfAgent)]
        self.agent = [Agent() for i in range(self.numberOfAgent)]

    def __initialize_agents__(self, population, hhold_nw, work_nw, school_nx, daycare_nx, THREAD):  # population dataset, hhold network, work network, School network,, daycare network
        logger.info("--SEIR->Initialize Agent..")
        ...
        for i in range(self.numberOfAgent):
            index_list.append(i)
            # ID
            self.agent[i].id = str(population.id[i])
            id_list.append(self.agent[i].id)
            # print(type(self.agent[i].id))
            # age
            self.agent[i].age = int(population.age[i])

            # status
            self.agent[i].status = 0  # set healthy at the initialization

            # hhold
            self.agent[i].hhold_id = population.hhold[i]# hhold id
            self.agent[i].hhold_size = hhold_nw.degree(self.agent[i].id)# hhold size
            self.agent[i].hhold_member = list(hhold_nw.neighbors(self.agent[i].id))# hhold member

            # work place
            self.agent[i].work_id = population.wp[i]
            ...
            if self.agent[i].age >= 18: #Adults
                if self.agent[i].work_id == self.agent[i].hhold_id:# work from home
                    ...
                    self.agent[i].work_size = work_nw.degree(self.agent[i].id)# work size
                    self.agent[i].work_member = list(work_nw.neighbors(self.agent[i].id))# work member
                ...
                if self.agent[i].work_id == self.agent[i].hhold_id:# stay at home
                    ...
                    self.agent[i].work_size = daycare_nx.degree(self.agent[i].id)# daycare nw size
                    self.agent[i].work_member = list(daycare_nx.neighbors(self.agent[i].id))# daycare network member

            elif (self.agent[i].age < 18) and (self.agent[i].age > 3):# kids go to school
                self.agent[i].work_size = school_nx.degree(self.agent[i].id)# school nw size
                self.agent[i].work_member = list(school_nx.neighbors(self.agent[i].id))# school network member

        id_dictionary = dict(zip(id_list, index_list))  # save all agent to dictionary
        #print('---Initialize ended ({:.1f} secs)'.format(timeit.default_timer() - start_time))

        # set infected agents (one or more)
        #hard code: selescrt three agent from Erie county and and set them as infected
        infected_list = ['36029012902i2526', '36029007304i568', '36029014803i3924']
        for item in infected_list:
            #print(item)
            agent_index = id_dictionary.get(item)
            #print("get agent using id", id_dictionary.get(item))
            self.agent[agent_index].status = 2  #change status to infected
            self.agent[agent_index].days = 1    # first day infected
            self.agent[agent_index].thread = THREAD  # the model starts with the orignial covid thread
            self.agent[agent_index].get_from = 101  # Patiant 0
            self.agent[agent_index].Set_Thread_Info()
            logger.info(f"Init_agent->The Infected Nodes is  {self.agent[agent_index].id}, "
                        f"status, {self.agent[agent_index].status}, "
                        f"thread, {self.agent[agent_index].thread} "
                        f"incubation, {self.agent[agent_index].incubation} "
                        f"spread speed, {self.agent[agent_index].spread_speed}")

        logger.info('---Total time for Initialize ({:.1f} secs)'.format(timeit.default_timer() - start_time))
                    ...
                    if (self.agent[a_index].status != 4 and
                        ...
                        if (self.agent[a_index].status == 0):  # only s can get infected
                            if self.agent[indi].r > 0:  # check if the indi agent still can spread the disease
                                ...
                                self.agent[indi].r = self.agent[indi].r - 1
    ...
    # =================================================================
    def Intro_New_Thread(self, DAY, INTRO):
    ...
    def get_infected_population(self,day):
        times = []
        id = []
        ...
        for agent in self.agent:
            if agent.status == 2:
                ...
                id.append(agent.id)
                #print(agent.id)
        print("I population:",len(id))
        df = pd.DataFrame([times, id]).transpose()
        df.columns = ['day', 'ids']
        #print(df.head())
